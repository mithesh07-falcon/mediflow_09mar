...
export default function PatientMedicationReportPage() {
  ...
  const downloadReport = (report: PrescriptionRecord) => {
    const lines = [
      `MediFlow Medication Report`,
      `Report ID: ${report.id}`,
      `Patient: ${report.patientName}`,
      `Doctor: ${report.doctorName} (${report.doctorSpecialization})`,
      `Date: ${report.date}`,
    ...
    createDownload(lines, `${report.id}-medication-report.txt`);
  };
      ...
      <main className="flex-1 p-8 bg-slate-50">
        <header className="mb-8 flex items-center justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-muted-foreground">Patient-side view of issued prescriptions and medication plans.</p>
          </div>
        </header>
        ...
        <Card className="rounded-2xl border-none shadow-sm">
          <CardHeader>
            <CardTitle className="text-xl">Available Medication Reports</CardTitle>
          </CardHeader>
                ...
                <div key={report.id} className="p-4 rounded-xl border bg-white flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2 text-sm text-muted-foreground">
                      ...
                      <Badge variant="outline" className="text-[10px]">{report.doctorSpecialization}</Badge>
                    </div>
